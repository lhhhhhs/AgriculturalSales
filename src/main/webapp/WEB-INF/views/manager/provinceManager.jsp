...
<div>
    ...
    <div class="layui-body" style="background-color: #F2F2F2">
        <!-- 内容主体区域 -->
        <table id="provinceTable" lay-filter="provinceTable"></table>
    </div>
    ...
    <form class="layui-form layui-form-pane" id="updateForm" lay-filter="updateForm"
        ...
        <div class="layui-form-item" pane>
            ...
            <div class="layui-input-block">
                <input type="text" name="provinceid" lay-verify="required|number" autocomplete="off" placeholder=""
                       class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="province" lay-verify="required" autocomplete="off" placeholder=""
                       class="layui-input">
            </div>
        </div>
...
<script>
    layui.use(['form', 'element', 'layer', 'carousel', 'jquery', 'table', 'laydate', 'layedit', 'upload'], function () {
        ...
        var tableIns = table.render({
            elem: '#provinceTable'
            , url: '${APP_PATH}/provinceTable' //数据接口
            , height: 550
            , page: true //开启分页
            , toolbar: '#toolBar'
            , cols: [[ //表头
                {field: 'id', title: 'ID', sort: true}
                , {field: 'provinceid', title: 'code'}
        ...
        //监听行工具事件
        table.on('tool(provinceTable)', function (obj) {
            var data = obj.data;
            console.log(obj);
            if (obj.event === 'del') {
        ...
        form.on('submit(doSubmit)', function (data) {
            $.post("${APP_PATH}/updateProvince", data.field, function (result) {
                if (result.code == 100) {
        ...
        form.on('submit(doAddSubmit)', function (data) {
            $.post("${APP_PATH}/addProvince", data.field, function (result) {
                if (result.code == 100) {
                    layer.alert(result.msg, {icon: 1});
                    //关闭窗口
        ...
        $(document).on('click', '#searchBtn', function () {
            //执行重载
            tableIns.reload({
                url: '${APP_PATH}/provinceTableSearch'//后台做模糊搜索接口路径
                , method: 'get'
                , where: {
