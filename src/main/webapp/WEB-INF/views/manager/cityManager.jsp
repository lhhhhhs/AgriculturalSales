...
<div>
    ...
    <div class="layui-body" style="background-color: #F2F2F2">
        <!-- 内容主体区域 -->
        <table id="cityTable" lay-filter="cityTable"></table>
    </div>
...
<div style="display: none" id="updateWin">
    <form class="layui-form layui-form-pane" id="updateForm" lay-filter="updateForm"
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="city" lay-verify="required" autocomplete="off" placeholder=""
                       class="layui-input">
            </div>
        </div>
        <div class="layui-form-item" pane>
            <label class="layui-form-label">省code</label>
            <div class="layui-input-block">
...
<script>
    layui.use(['form', 'element', 'layer', 'carousel', 'jquery', 'table', 'laydate', 'layedit', 'upload'], function () {
        ...
        var tableIns = table.render({
            elem: '#cityTable'
            , url: '${APP_PATH}/cityTable' //数据接口
            , height: 550
            , page: true //开启分页
            , toolbar: '#toolBar'
            , cols: [[ //表头
                {field: 'id', title: 'ID', sort: true}
                , {field: 'cityid', title: 'code'}
                , {field: 'city', title: '名称'}
                , {field: 'provinceid', title: '省code'}
        ...
        form.on('submit(doSubmit)', function (data) {
            $.post("${APP_PATH}/updateCity", data.field, function (result) {
                if (result.code == 100) {
        ...
        form.on('submit(doAddSubmit)', function (data) {
            $.post("${APP_PATH}/addCity", data.field, function (result) {
                if (result.code == 100) {
                    layer.alert(result.msg, {icon: 1});
                    //关闭窗口
        ...
        $(document).on('click', '#searchBtn', function () {
            //执行重载
            tableIns.reload({
                url: '${APP_PATH}/cityTableSearch'//后台做模糊搜索接口路径
                , method: 'get'
                , where: {
