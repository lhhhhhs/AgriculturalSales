    ...
    <c:forEach items="${addressList}" var="address">
        <div class="panel panel-default" style="margin-left: 100px;margin-right: 100px;margin-top: 20px;">
            ...
            <div class="panel-body orderBody">
                <input type="checkbox" name="header" onclick="checkboxOnclick(${address.id})" style="float: left;margin-right: 20px;" <c:if test="${address.id==addressId}">checked="checked"</c:if>>
                <span style="margin-right: 10px;float: left">地址：${address.province}${address.city}${address.area}${address.detailed}</span>

                <a href="${APP_PATH}/delAddress?addressId=${address.id}&orderId=${id}" class="btn btn-danger"
                   style="float: right;margin-right: 10px;">删除地址</a>
                <a href="${APP_PATH}/editAddress?addressId=${address.id}&orderId=${id}" class="btn btn-success" style="float: right;margin-right: 10px;">编辑</a>
                <a href="${APP_PATH}/defaultAddress?addressId=${address.id}&orderId=${id}" class="btn btn-success" style="float: right;margin-right: 10px;">设为默认地址</a>
